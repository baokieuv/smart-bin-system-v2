...
export default function ShopHomePage() {
  ...
  const { pushToast: pushToastLocal, ToastContainer: ToastContainerLocal } = useToast();

  const handleQuickAdd = async (productId: string, sku: string | undefined) => {
    if (!sku) {
      pushToastLocal('Không thể thêm vào giỏ: Sản phẩm không có mã SKU', 'error');
      return;
    }
    ...
    if (!isLoggedIn) {
      ...
      pushToastLocal('Đã thêm vào giỏ tạm thời. Bạn có thể checkout sau khi đăng nhập.', 'success');
      return;
    }
    ...
    try {
      ...
      pushToastLocal('Đã thêm vào giỏ hàng', 'success');
    } catch (error) {
      pushToastLocal(error instanceof Error ? error.message : 'Thêm vào giỏ hàng thất bại', 'error');
    }
  };
      ...
      <div className="space-y-5">
            ...
            <div className="relative grid gap-6 p-6 sm:p-8 lg:grid-cols-[1.1fr_0.9fr]">
              <div className="space-y-5">
                ...
                <p className="max-w-xl text-sm leading-6 text-slate-600 sm:text-base">
                  Danh mục mở cho mọi người, hiển thị rõ giá và tồn kho. Đăng nhập để thêm giỏ, thanh toán, và theo dõi lịch sử đơn.
                </p>
              ...
              <div className="flex flex-col justify-between gap-4">
                <div className="overflow-hidden rounded-3xl bg-slate-900">
                  {featured.imageUrl ? (
                    <img src={featured.imageUrl} alt={featured.name || featured.title || 'Featured product'} className="h-48 w-full object-cover sm:h-56" />
                  ) : (
                    <div className="flex h-48 items-center justify-center text-sm font-semibold text-slate-200">Featured preview</div>
                  )}
                </div>
        ...
        <Surface className="border-0 bg-white/80 p-4 shadow-[0_16px_40px_-32px_rgba(15,23,42,0.4)] sm:p-5">
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div className="min-w-0 flex-1">
                  <Input
                    ...
                    placeholder="Nhập tên, mô tả, hoặc từ khóa..."
                  />
                </div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3 text-xs font-semibold text-slate-500">
            ...
            {state.message ? (
              <span className="rounded-full bg-amber-50 px-3 py-1 text-amber-800">{state.message}</span>
            ) : null}
          </div>
        </Surface>
              ...
              <Surface key={product.id} className="overflow-hidden border-0 bg-white/90 shadow-[0_16px_40px_-32px_rgba(15,23,42,0.4)]">
                <div className="relative aspect-4/3 bg-slate-100">
                  {image ? (
                    ...
                    <div className="flex h-full items-center justify-center bg-linear-to-br from-emerald-100 to-amber-100 text-sm font-semibold text-slate-600">
                      Product preview
                    </div>
                  )}
                ...
                <div className="space-y-3 p-5">
                  ...
                  <p className="line-clamp-2 text-sm leading-6 text-slate-600">
                    {product.description || product.shortDescription || 'Xem chi tiết để biết thông số, tính năng và tình trạng hàng.'}
                  </p>
                  ...
                  <div className="flex flex-col gap-2 sm:flex-row">
                    <Link
                      ...
                      className="inline-flex flex-1 items-center justify-center rounded-full bg-white/90 px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-white"
                    >
                      Chi tiết
                    </Link>
                    <Button
                      ...
                      onClick={() => void handleQuickAdd(product.id, product.sku)}
                    >
                      Thêm vào giỏ
                    </Button>
                  </div>
      ...
      <div className="space-y-5">
        ...
        <Surface className="border-0 bg-white/80 p-5 shadow-[0_16px_40px_-32px_rgba(15,23,42,0.4)]">
          ...
          <div className="mt-4 space-y-3 text-sm text-slate-600">
            <div className="rounded-2xl bg-slate-50 px-4 py-3">Cập nhật tồn kho theo thời gian thực từ dashboard vận hành.</div>
