...
import { formatCurrency, formatDateTime, toNumber, unwrapListPayload, useValidAuthToken } from '@/lib/shop-utils';
import type { CartLineDto, CartSummaryDto, OrderDetailDto } from '@/types/shop';

type CartState = { status: 'loading' | 'ready' | 'error'; message?: string; summary?: CartSummaryDto; items: CartLineDto[] };
type OrderState = { status: 'loading' | 'ready' | 'error'; message?: string; orders: OrderDetailDto[] };
...
export default function CartPage() {
  ...
  useEffect(() => {
    ...
    const loadCart = async () => {
      ...
      if (!isLoggedIn) {
        setCartState({ status: 'ready', summary: undefined, items: toCartLinesFromGuest() });
        return;
      }
      ...
      try {
        ...
        setCartState({ status: 'error', message: errorMsg, items: [] });
    ...
    const loadOrders = async () => {
      ...
      try {
        ...
        setOrderState({ status: 'error', message: errorMsg, orders: [] });
        pushToast(errorMsg, 'error');
      }
    };
  ...
  const totals = useMemo(() => {
    ...
    return { subtotal: base, shippingFee, discount, total };
  ...
  const updateQuantity = async (sku: string | undefined, quantity: number) => {
    ...
    if (!isLoggedIn) {
      ...
      setCartState((current) => ({ ...current, status: 'ready', items: toCartLinesFromGuest() }));
      setUpdatingSku(null);
      return;
    }
    ...
    try {
      ...
      setCartState((current) => ({ ...current, status: 'error', message: errorMsg }));
      pushToast(errorMsg, 'error');
    } finally {
      setUpdatingSku(null);
    }
  ...
  const handleCheckout = async () => {
    ...
    try {
      const response = await shopApi.checkout({ shippingAddress: shippingAddress.trim(), paymentMethod });
      if (!response.success) {
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[1.4fr_0.9fr]">
      ...
      <Surface className="border-0 bg-white/85 p-5 shadow-[0_18px_50px_-32px_rgba(15,23,42,0.45)] sm:p-6">
        ...
        {cartState.message ? <div className="mt-4 rounded-2xl bg-amber-50 px-4 py-3 text-sm text-amber-800">{cartState.message}</div> : null}
                ...
                <div key={`${item.productSku || title}-${index}`} className="flex flex-col gap-4 rounded-2xl bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <div className="flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1">
                      <button type="button" onClick={() => updateQuantity(item.productSku, Math.max(1, quantity - 1))} disabled={!item.productSku || updatingSku === item.productSku} className="h-6 w-6 rounded-full bg-white text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50">
                        -
                      </button>
                      <span className="min-w-6 text-center text-sm font-semibold text-slate-700">{quantity}</span>
                      <button type="button" onClick={() => updateQuantity(item.productSku, quantity + 1)} disabled={!item.productSku || updatingSku === item.productSku} className="h-6 w-6 rounded-full bg-white text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50">
                        +
                      </button>
                    </div>
      ...
      <div className="space-y-5">
        <Surface className="border-0 bg-white/85 p-5 shadow-[0_18px_50px_-32px_rgba(15,23,42,0.45)] sm:p-6">
          ...
          <div className="mt-4 space-y-3 text-sm">
            <div className="flex items-center justify-between"><span className="text-slate-600">Tạm tính</span><span className="font-semibold text-slate-900">{formatCurrency(totals.subtotal)}</span></div>
            <div className="flex items-center justify-between"><span className="text-slate-600">Phí vận chuyển</span><span className="font-semibold text-slate-900">{formatCurrency(totals.shippingFee)}</span></div>
            <div className="flex items-center justify-between"><span className="text-slate-600">Giảm giá</span><span className="font-semibold text-amber-700">- {formatCurrency(totals.discount)}</span></div>
            <div className="flex items-center justify-between border-t border-slate-200/70 pt-3"><span className="text-slate-900">Tổng cộng</span><span className="text-lg font-bold text-slate-900">{formatCurrency(totals.total)}</span></div>
          </div>

          <div className="mt-5 space-y-3">
            <div>
              <label className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Địa chỉ giao hàng</label>
              <Input value={shippingAddress} onChange={(event) => setShippingAddress(event.target.value)} placeholder="Nhập địa chỉ đầy đủ..." className="mt-2" />
            </div>
            ...
            {checkoutMessage ? <div className="rounded-2xl bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-800">{checkoutMessage}</div> : null}

            <Button onClick={() => void handleCheckout()} disabled={isCheckingOut || cartState.items.length === 0} className="w-full rounded-full">
        ...
        <Surface className="border-0 bg-white/85 p-5 shadow-[0_18px_50px_-32px_rgba(15,23,42,0.45)] sm:p-6">
          ...
          {!isLoggedIn ? (
            <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">Đăng nhập để xem lịch sử đơn hàng.</div>
          ) : orderState.status === 'loading' ? (
            <p className="mt-4 text-sm text-slate-600">Đang tải đơn hàng...</p>
          ) : orderState.orders.length === 0 ? (
            <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">Chưa có đơn hàng nào gần đây.</div>
          ) : (
            <div className="mt-4 space-y-3">
              {orderState.orders.map((order) => (
                <Link key={order.id} href={`/shop/orders/${order.id}`} className="flex items-center justify-between rounded-2xl bg-white px-4 py-3 shadow-sm transition hover:bg-slate-50">
                  <div>
