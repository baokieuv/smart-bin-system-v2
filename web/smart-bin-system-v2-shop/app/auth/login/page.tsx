...
export default function LoginPage() {
  ...
  const redirectAfterLogin = async () => {
    const safeReturnUrl = (() => {
      ...
      return returnUrl && returnUrl.startsWith('/') ? returnUrl : '/shop';
    })();
  ...
  return (
    <AuthShell title="Welcome Back" description="Sign in to continue shopping and keep your cart in sync.">
      {ToastContainer}
      ...
      <form onSubmit={handlePasswordLogin} className="space-y-5">
        ...
        <div>
          ...
          <div className="relative">
            <Input type={showPassword ? 'text' : 'password'} value={password} onChange={(event) => setPassword(event.target.value)} className="pr-10" required />
      ...
      <p className="mt-5 text-center text-sm text-slate-600">
        Back to <Link href="/shop" className="font-semibold text-emerald-700 hover:underline">shop</Link>
      </p>
    </AuthShell>
  );
}
