...
import { inventoriesAdminApi } from "@/services/api/inventories-admin";

type InventoryItem = { sku: string; quantity: number };

export default function ImportInventoryPanel({ onImported }: { onImported?: () => void }) {
  ...
  const parseFile = async (file: File) => {
    ...
    try {
      ...
      if (rows.length > 0 && typeof rows[0] === "object" && !Array.isArray(rows[0])) {
        ...
        const qtyKey = keys.find((k) => /quantity|qty|stock/.test(k.toLowerCase()));
        ...
        for (const r of rows as unknown[]) {
          ...
          const sku = String(arr[0] ?? "").trim();
          const qty = Number(arr[1] ?? 0) || 0;
          if (sku && qty > 0) items.push({ sku, quantity: qty });
        }
      }
              ...
              <tbody>
                {preview.slice(0, 200).map((p, idx) => (
                  <tr key={idx} className="border-t border-slate-100">
                    ...
                    <td className="py-1 text-slate-600">{p.quantity}</td>
                  </tr>
                ))}
              </tbody>
            </table>
